...
- [Lab Setup Verification](#lab-setup-verification)
  ...
  - [Step 3: SSH Access to the Node That Is Successful](#step-3-ssh-access-to-the-node-that-is-successful)
- [Exercise 1: Tracing a BGP Route End-to-End Through the SONiC Layers](#exercise-1-tracing-a-bgp-route-end-to-end-through-the-sonic-layers)
  - [Task 1.1: Pick a Remote Prefix and Validate It Is in BGP](#task-11-pick-a-remote-prefix-and-validate-it-is-in-bgp)
...
- [Exercise 2: iptables, Control-Plane ACL, and Data-Plane ACL in SONiC](#exercise-2-iptables-control-plane-acl-and-data-plane-acl-in-sonic)
  ...
  - [Task 2.4: Data-Plane ACL (Optional)](#task-24-data-plane-acl-optional--do-this-exercise-if-241-works)
  - [Task 2.5: Cleanup — Use the Config Checkpoint (Optional)](#task-25-cleanup--use-the-config-checkpoint-optional)
- [Exercise 3: Stream Telemetry with gNMI](#exercise-3-stream-telemetry-with-gnmi)
  - [Task 3.1: Validate gnmic Collector on the Host](#task-31-validate-gnmic-collector-on-the-host)
  - [Task 3.2: Validate gNMI Is Enabled on the SONiC Device - Leaf00](#task-32-validate-gnmi-is-enabled-on-the-sonic-device---leaf00)
  ...
  - [Task 3.6: Streaming Data via gNMI](#task-36-streaming-data-via-gnmi)
- [Exercise 4: Troubleshoot Like a Pro (Optional)](#exercise-4-troubleshoot-like-a-pro-optional)
...
Use your PuTTY session to land on the virtual host
...
> **Note** - In some scenarios, some of the nodes may not respond due to resource constraints. Lab has been structured with minimal nodes. Note the name of nodes that came back successful. If the output hangs, press `control c`
...
## Exercise 1: Tracing a BGP Route End-to-End Through the SONiC Layers
...
### Task 1.1: Pick a Remote Prefix and Validate It Is in BGP

```bash
ssh admin@leaf00
#From leaf00 - i am picking the lo0 prefix on leaf01
...
- This is an ECMP path; we will focus on the best route.
...
-  ``zebra` is the process in the BGP container that selects the BGP path and resolves the next-hops **`10.1.1.0`** / **`10.1.1.4`** out of **`Ethernet0`** / **`Ethernet8`**.
...
- This shows that `zebra` installed the route into the Linux kernel via Netlink. The kernel routes are used for software forwarding when hardware forwarding fails. From here, the **`fpmsyncd`** daemon mirrors the FIB into Redis.

---

### Task 1.4: Trace into APPL_DB (Redis DB 0)

`fpmsyncd` is the process within the `bgp` container that writes the route as a hash in `ROUTE_TABLE`.
...
- The key to use to gather details around 10.0.0.3 is Route_Table. Ideally, you will grep for destination prefix and find the keys
...
Watch `orchagent`, a process in the `swss` container, translate the route into ASIC objects. `orchagent` subscribes to `APPL_DB:ROUTE_TABLE`, resolves the next-hop into a SAI Next-Hop Group, and writes SAI objects to `ASIC_DB`.
...
```
>> **Note** - Actual OID may vary. Use the value you obtained in the table as the `oid`.
...
```
>> **Note** - Replace the OID with the output from Task 1.5.2
...
- If you have entry in above, you can skip this next few steps.
...
#### Task 1.5.4: List All Group-Member Keys (Optional)
...
```
>> **Note** - Actual OID may vary with the setup. Please use this as a reference.
...
##### Task 1.5.4.1: Inspect Each Member and Keep the Ones Whose Group Matches (Optional)

```
redis-cli -n 1 HGETALL "ASIC_STATE:SAI_OBJECT_TYPE_NEXT_HOP_GROUP_MEMBER:oid:0x2d0000000004ed"
```
>> **Note** - Replace the 1st OID with the output from Task 1.5.4
...
```
**Observation**
- If you are getting `(empty array)` that means you are typing in wrong oid for NEXT_HOP_GROUP_Member
- NEXT_HOP_GROUP_ID" Does not match with group oid - oid:0x50000000004e9" that was displayed with Task 1.5.2
- If it matches go to next section


Replace the 2nd OID with the output from Task 1.5.4
...
**Observation**
- If you are getting `(empty array)` that means you are typing in wrong oid for NEXT_HOP_GROUP_Member
- NEXT_HOP_GROUP_ID" Matches with group oid - oid:0x50000000004e9" that was displayed with Task 1.5.2
- If it matches go to next section

Replace the 3rd OID with the output from Task 1.5.4
...
Replace the 4th OID with the output from Task 1.5.4
...
#### Task 1.5.5: Inspect the SAI_NEXT_HOP_GROUP_MEMBER_ATTR_NEXT_HOP_ID for the Matching Group (Optional)
...
#### Task 1.5.7: Find Out the Next-Hop Interface from "SAI_ROUTER_INTERFACE_ATTR_PORT_ID"

Grep the  `SAI_ROUTER_INTERFACE_ATTR_PORT_ID` from above
...
> **Note:** You have now followed the route from APPL_DB → ASIC_DB → SAI Next-Hop → Router Interface → physical port (`Ethernet0`).
...
## Exercise 2: iptables, Control-Plane ACL, and Data-Plane ACL in SONiC
...
**Observation**
- We have taken a configuration snapshot prior to making changes to the network. You can always restore it by using `config rollback <checkpoint>`.
...
```
Copy paste the following in the vi editor.
 - "i" for insert edit mode
 - copy/paste from following
 - esc :wq! to save and quit (esc needed to come back to control mode)
...
> **Note:** You can use any editor you are comfortable with to create an ACL rule.
...
```
**Observation**
The custom config file is loaded into the running CONFIG_DB (DB 4).
...
- The configuration is loaded into the running instance.
- Run `sudo config save -y` to permanently store it in the startup configuration.

#### Task 2.3.3: Save and take a snapshot with post change

Take a backup of the configuration using `config checkpoint`:

```bash
sudo config checkpoint SSH-ACL-configs
```
**Expected Output:**

```text
Config Rollbacker: Config checkpoint starting.
Config Rollbacker: Checkpoint name: SSH-ACL-configs.
Config Rollbacker: Getting current config db.
Config Rollbacker: Getting checkpoint full-path.
Config Rollbacker: Ensuring checkpoint directory exist.
Config Rollbacker: Saving config db content to /etc/sonic/checkpoints/SSH-ACL-configs.cp.json.
Config Rollbacker: Config checkpoint completed.
Checkpoint created successfully.
```
...
#### Task 2.3.4: Verify It Landed in iptables
...
#### Task 2.3.5: Diff and validate there is change in configuration
...
#### Task 2.3.6: Functional Test (Optional)

From a jump host (in **172.20.2.0/24**):

```bash
# cisco@clab:~$ - from the Host
ssh admin@leaf00      # should succeed
```

#### Task 2.3.7: Prove This Is **Not** in TCAM
...
```
**Observation**
- The ACL is not programmed in the data plane, as packet counts are N/A.

---

### Task 2.4: Data-Plane ACL (Optional — Do This Exercise If 2.4.1 Works)

Advertise access network in LEAF00 & LEAF01 if needed. { Optional step}
...
**Observation**
- End-to-end traffic works, transiting from Leaf00 to Leaf01 via the spine.

> **Note:** If the test above fails, you cannot validate the data plane due to topology issues. Check if the route is advertised in BGP. Reach out to a proctor.

#### Task 2.4.2: Configure the Custom File in JSON Format

Create `/tmp/acl_dataplane_ICMP.json` on the **ACL leaf00** (the device whose `Ethernet0` faces the client):
...
> **Note:** Use the editor of your choice if you are not comfortable with VI/VIM.
...
```
Running command: /usr/local/bin/sonic-cfggen -j /tmp/acl_dataplane_ICMP.json --write-to-db
...
```
**Observation**
- Counters will increase if traffic is being blocked.
...
```
**Observation**
- Notice that the block counters are increasing.
...
## Exercise 3: Stream Telemetry with gNMI

### Task 3.1: Validate gnmic Collector on the Host

gnmic works with both OpenConfig and SONiC native models.

> **Note:** Support for configuration via gNMI is limited. This lab focuses on operational data for collecting telemetry from the SONiC host.
...
### Task 3.2: Validate gNMI Is Enabled on the SONiC Device - Leaf00
...
```bash
#clab-host
gnmic -a 172.20.2.101:8080 --insecure capabilities
```
...
**Observation**
 ...
 - Shows the list of negotiated models between the collector and the server.

> **Note** All supported models within the platform can be seen in the `/usr/models/yang/` folder.
...
gnmic -a 172.20.2.101:8080 --insecure -u admin -p password get --path "/openconfig-system:system/state/software-version"

gnmic -a 172.20.2.101:8080 --insecure -u admin -p password get --target CONFIG_DB --path "/VERSIONS"
```
...
**Observation**
- shows the equivalent of show version CLI via ssh channel or session
- The version can be gathered via OpenConfig or by querying CONFIG_DB.
...
```bash
 gnmic -a 172.20.2.101:8080 --insecure -u admin -p password get --path "/openconfig-interfaces:interfaces/interface[name=Ethernet0]"
 ```
...
[
  {
    "source": "172.20.2.101:8080",
    "timestamp": 1780100885236527272,
    "time": "2026-05-30T00:28:05.236527272Z",
    "updates": [
      {
...
- Check with vendor or device capabilities on supported models with the SONiC Versions

### Task 3.6: Streaming Data via gNMI

```bash
gnmic -a 172.20.2.101:8080 --insecure subscribe --path "COUNTERS/Ethernet0*/SAI_PORT_STAT_IF_IN_ERRORS" --target COUNTERS_DB --stream-mode sample --sample-interval 5s --format flat
```

**Expected Output:**
```

/COUNTERS/Ethernet16*/SAI_PORT_STAT_IF_IN_ERRORS/etp0/SAI_PORT_STAT_IF_IN_ERRORS: 0
/COUNTERS/Ethernet16*/SAI_PORT_STAT_IF_IN_ERRORS/etp1/SAI_PORT_STAT_IF_IN_ERRORS: 0
/COUNTERS/Ethernet16*/SAI_PORT_STAT_IF_IN_ERRORS/etp10/SAI_PORT_STAT_IF_IN_ERRORS: 0
/COUNTERS/Ethernet16*/SAI_PORT_STAT_IF_IN_ERRORS/etp11/SAI_PORT_STAT_IF_IN_ERRORS: 0
/COUNTERS/Ethernet16*/SAI_PORT_STAT_IF_IN_ERRORS/etp12/SAI_PORT_STAT_IF_IN_ERRORS: 0
/COUNTERS/Ethernet16*/SAI_PORT_STAT_IF_IN_ERRORS/etp13/SAI_PORT_STAT_IF_IN_ERRORS: 0
/COUNTERS/Ethernet16*/SAI_PORT_STAT_IF_IN_ERRORS/etp14/SAI_PORT_STAT_IF_IN_ERRORS: 0
/COUNTERS/Ethernet16*/SAI_PORT_STAT_IF_IN_ERRORS/etp15/SAI_PORT_STAT_IF_IN_ERRORS: 0
/COUNTERS/Ethernet16*/SAI_PORT_STAT_IF_IN_ERRORS/etp16/SAI_PORT_STAT_IF_IN_ERRORS: 0
/COUNTERS/Ethernet16*/SAI_PORT_STAT_IF_IN_ERRORS/etp17/SAI_PORT_STAT_IF_IN_ERRORS: 0
...<snip>
```
**Observation**

- Streaming data from the Redis DB.
---

## Exercise 4: Troubleshoot Like a Pro (Optional)

### Objective

Get familiar with the SONiC logging stack and learn where to look — and what to collect — when something goes wrong. By the end of this exercise you will know which tool surfaces which type of event, how to scope a log search by time or service, and how to package an artifact bundle for Cisco TAC.

### Table of Contents

- [Task 4.1: `journalctl` — The Boot and Systemd View](#task-41-journalctl-the-boot-and-systemd-view)
    - [Task 4.1.1: Boot and Service Status (Optional)](#task-411-boot-and-service-status-optional)
    - [Task 4.1.2: Time-Bounded Search (Optional)](#task-412-time-bounded-search-optional)
- [Task 4.2: `/var/log/syslog` — The Unified Application Log](#task-42-varlogsyslog-the-unified-application-log)
    - [Task 4.2.1: Tail and Filter (Optional)](#task-421-tail-and-filter-optional)
    - [Task 4.2.2: Rotated Logs (Optional)](#task-422-rotated-logs-optional)
- [Task 4.3: Per-Container Logs with `docker logs`](#task-43-per-container-logs-with-docker-logs)
    - [Task 4.3.1: List Containers](#task-431-list-containers)
    - [Task 4.3.2: Pull Logs From One Container](#task-432-pull-logs-from-one-container)
    - [Task 4.3.3: Follow `swss` Live (Optional)](#task-433-follow-swss-live-optional)
    - [Task 4.3.4: FRR-Specific Logs Inside the `bgp` Container (Optional)](#task-434-frr-specific-logs-inside-the-bgp-container-optional)
- [Task 4.4: `show logging` and Component Log Levels](#task-44-show-logging-and-component-log-levels)
    - [Task 4.4.1: Inspect Current Levels (Optional)](#task-441-inspect-current-levels-optional)
    - [Task 4.4.2: Bump a Single Component to DEBUG (Optional)](#task-442-bump-a-single-component-to-debug-optional)
- [Task 4.5: `show techsupport` — The TAC Bundle](#task-45-show-techsupport-the-tac-bundle)
    - [Task 4.5.1: Generate the Bundle (Optional)](#task-451-generate-the-bundle-optional)
    - [Task 4.5.2: What's Inside (Optional)](#task-452-whats-inside-optional)

> **Note:** The tasks in this section are marked optional. Run these commands to get familiar with the platform's behavior. The device may reboot while running resource-intensive commands.
...
#### Task 4.3.3: Follow `swss` Live (Optional)

```bash
docker logs -f swss
```

#### Task 4.3.4: FRR-Specific Logs Inside the `bgp` Container (Optional)
...
```bash
# Verify the current logging level
sudo swssloglevel -p
